...
`temperature`는 프롬프트와 출력 사이의 결합도에도 영향을 줍니다. 프롬프트가 구체적인 형식·근거·제약을 요구할수록 낮은 `temperature`에서 지시와 출력의 결합도가 높아져 일관된 결과를 얻기 쉽고, 높은 값에서는 같은 프롬프트에도 표현과 구성의 변동이 커질 수 있습니다.
